-- Align order-time price validation with the app's displayed prices.
-- The client bakes active automatic promotions (scope all/category/coffee)
-- into the cart's unit_price, but order_item_unit_price validated against the
-- undiscounted base_price, so checkout failed with
-- 'Menu price changed; refresh your cart' whenever a promotion was running.
--
-- Resolution mirrors src/utils/pricing.ts getCoffeeDiscount exactly:
--   active + date range + store, priority coffee > category > all,
--   most recently created first within the same scope.
