...
drop function if exists public.create_order(uuid, text, numeric, numeric, numeric, jsonb, numeric, text, numeric, numeric, text, boolean);
...
begin
  ...
  if p_idempotency_key is not null and length(trim(p_idempotency_key)) > 100 then
  ...
  if nullif(trim(p_promo_code), '') is not null then
    ...
    if promo_row.scope <> 'all' then raise exception 'This promotion scope is not supported by checkout'; end if;
  ...
  for item in select value from jsonb_array_elements(p_items) loop
    ...
    insert into public.order_items (
      ...
      (item->>'quantity')::integer, item_price, (item->>'compare_at_price')::numeric
    );
  end loop;
  return order_id;
exception
  when unique_violation then
    ...
    where user_id = auth.uid() and idempotency_key = nullif(trim(p_idempotency_key), '')
    limit 1;
    if existing_order_id is not null then return existing_order_id; end if;
    raise;
end;
...
revoke all on function public.create_order(uuid, text, numeric, numeric, numeric, jsonb, numeric, text, numeric, numeric, text, boolean, text) from public;
grant execute on function public.create_order(uuid, text, numeric, numeric, numeric, jsonb, numeric, text, numeric, numeric, text, boolean, text) to authenticated;
