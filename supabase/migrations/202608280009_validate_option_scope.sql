...
begin
    ...
    if option_label is not null then
      if not exists (
        ...
        where o.store_id = p_store_id
          ...
          and o.label = option_label
          and (
            not exists (select 1 from public.coffee_option_categories oc where oc.option_id = o.id)
            or exists (
              select 1 from public.coffee_option_categories oc
              where oc.option_id = o.id and oc.category_id = coffee_row.category_id
            )
          )
      ...
      option_total := option_total + coalesce((
        ...
        where o.store_id = p_store_id and o.type = option_type and o.label = option_label
          and (
            not exists (select 1 from public.coffee_option_categories oc where oc.option_id = o.id)
            or exists (select 1 from public.coffee_option_categories oc where oc.option_id = o.id and oc.category_id = coffee_row.category_id)
          )
        limit 1
      ), 0);
    end if;
  ...
  for option_label in select jsonb_array_elements_text(coalesce(p_item->'extras', '[]'::jsonb)) loop
    if not exists (
      select 1 from public.coffee_options o
      where o.store_id = p_store_id and o.type = 'extra' and o.label = option_label
        and (
          not exists (select 1 from public.coffee_option_categories oc where oc.option_id = o.id)
          or exists (select 1 from public.coffee_option_categories oc where oc.option_id = o.id and oc.category_id = coffee_row.category_id)
        )
